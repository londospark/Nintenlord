...
namespace Nintenlord.Parser
{
    ...
    public static class MonadicParserHelpers
    {
        ...
        public static MonadicParser<TIn, TOut> Always<TIn, TOut>() => scanner => Tuple.Create(default(TOut), new Match<TIn>(scanner, 0));

        public static MonadicParser<TIn, TOut> Never<TIn, TOut>() => scanner => Tuple.Create(default(TOut), new Match<TIn>(scanner, "Epic fail."));

        public static IParser<TIn, TOut> GetParser<TIn, TOut>(this MonadicParser<TIn, TOut> parser) => new FunctionParser<TIn, TOut>(x => parser(x));
